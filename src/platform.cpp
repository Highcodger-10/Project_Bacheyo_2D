...
{
	this->platformImage.loadFromFile("/home/highcodger_10/Desktop/Project_OOP_bachiyo_2D/images/ground & platforms/platform.png");
...
void platform::initPositions(int WorldMap[][1000])
{
	for (int i = 0;i < 10;i++)
	{
		for (int j = 0; j < 1000; j++)
		{
			if (WorldMap[i][j] == 2)
			{
				this->platforms[i][j].setPosition(60*j , 500 - 50 * (8 - i));
	...
	for (int i = 0;i < 10;i++)
	{
		for (int j = 0;j < 1000;j++)
		{
			...
			if (chara.collision)
			{
				...
				if (xvel > 0 && 
					(chara.character.getPosition().y - 15.f < this->platforms[i][j].getPosition().y) &&
					(chara.character.getPosition().x + chara.character.getGlobalBounds().width - 5.f) > this->platforms[i][j].getPosition().x)
				{
				...
				if (xvel < 0 and
					(chara.character.getPosition().y - 15.f < this->platforms[i][j].getPosition().y) &&
					chara.character.getPosition().x - 10.f < (this->platforms[i][j].getPosition().x + this->platforms[i][j].getGlobalBounds().width))
				{
				...
				if (yvel < 0 &&
				(chara.character.getPosition().x + 40.f < (this->platforms[i][j].getPosition().x + this->platforms[i][j].getGlobalBounds().width + 10.f)))
				{
